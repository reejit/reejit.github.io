...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const texts = [
        { text: "Am I retarded?" },
        { text: "Loves kalesh" },
        { text: "Professional Yapper" },
        { text: "Hawww stalker!!", tag: "i" }
    ];
    ...
    elements.forEach((element) => {
        ...
        function typeWriter() {
            const { text, tag } = texts[currentIndex];
            const visibleText = text.substring(0, textIndex);
            element.innerHTML = tag ? `<${tag}>${visibleText}</${tag}>` : visibleText;
